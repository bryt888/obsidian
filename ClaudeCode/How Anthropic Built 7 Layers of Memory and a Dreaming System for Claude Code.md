...
Summary
Claude Code faces bounded 200K token context windows that coding sessions quickly exceed with file reads, grep, and edits.
It deploys a 7-layer memory architecture starting with cheap token pruning and tool result storage on disk using previews in <persisted-output> tags.
Microcompaction clears old tool results via time-based, cache-edits, or API-level methods to preserve prompt cache hits.
Session memory maintains ongoing markdown notes for cheap compaction; full compaction forks a summarizer when needed, reserving 20K tokens.
Auto memory extraction builds durable cross-session knowledge while dreaming consolidates past sessions in background like biological sleep. 
Forked agents enable isolation with shared prompt caches; layered design and feature flags ensure cheapest layers prevent expensive ones.
...
10. Layer 5: Auto Memory Extraction
...
10. Full parent assistant message (all tool_use blocks, thinking, text)
11. Single user message with:
...
12. CacheSafeParams: Every forked agent (session memory, compaction, dreaming, extraction) inherits the parent's exact system prompt, tools, and message prefix. The fork's API request has an identical prefix → cache hit.
13. renderedSystemPrompt: The fork threads the parent's already-rendered system prompt bytes, avoiding re-rendering divergence (e.g., GrowthBook flag value changes between renders).
14. ContentReplacementState cloning: Tool result persistence decisions are frozen. The same results get the same previews on every API call → stable prefix.
15. Cached microcompact: Uses cache_edits to modify the server cache without changing the local prefix → no cache break.
16. Fork message construction: All fork children get byte-identical prefixes. Only the final directive differs → maximum cache sharing across concurrent forks.
17. Post-compact cache break notification: After compaction, notifyCompaction() resets the cache baseline so the expected post-compact cache miss isn't flagged as an anomaly.
...
20. Layered Defense, Cheapest First
...
@troyhua
Building Agentic Memory at 
@EverMind
 PhD on Dialogue System at CMU